...
#def eval(epo):
r_his = []
with open("IRL_pretest_result_pedestrian.json", "a")  as f2,open("info_car2.json", "a") as car2 ,open("info_pedestrian1.json", "a") as pedestrian1:

    for j in range(20): #(5 epo):
        ...
        while i<1500: #1000 #while not done:
            ...
            #with tf.Session() as sess:
            # saver = tf.train.import_meta_graph(path_checkpoint+'/49.meta')
            # saver.restore(sess, latest_checkpoint)
            # graph = tf.get_default_graph()
            ...
            py_measurements["action"] = action['car2']
            ...
            next_obs, reward, done, info = env.step({'car2': action['car2'], 'pedestrian1': action['pedestrian1']})#,noise=False)
            ...
            for agent_id in reward:
                ...
                if agent_id=="pedestrian1":
                    json_dump = json.dumps(info[agent_id], cls=NumpyEncoder)
                    pedestrian1.write(json_dump)
                    pedestrian1.write("\n")
