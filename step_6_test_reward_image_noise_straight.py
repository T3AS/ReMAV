...
#def eval(epo):
r_his = []
with open("IRL_protest_result_imagenoise_straight.json", "a")  as f2, open("info_car1.json", "a") as car1:
    ...
    for i in range(20): #(5 epo):
        ...
        while i<1500: #1000 #while not done:
            ...
            if  (output < -8.0):# or output > -4.50):
                foundBool = True
                print ("Found it !!!!!!!!!!!!!")
                gauss_noise=np.zeros((168,168,3),dtype=np.float32)
                cv2.randn(gauss_noise,0,0.01)
            ...
            py_measurements["action"] = action['car1']
            ...
            next_obs, reward, done, info = env.step(action_dict={'car1': action['car1']})#,noise=False)
